/**
 * @title   sched_queue.cpp
 * @module  实现等待队列和服务队列对象的类中方法
 * @author  于越
...
bool WaitQueue::add(QString roomId, int fanSpeed, int waitTime){
    ...
    QueueType tmp(roomId,fanSpeed,waitTime,nowTime);
    roomList.append(tmp);
    ...
    if(res_large.length()!=0){/*大于*/
        if(res_large.length()==1){/*只有一个低于*/
            ...
            if(tmpspeed==minspeed){/*多个低于且相等，用服务时长最大的*/
                ...
                QString minroom=res_large[0].roomId;
                for(int i=0;i<res_large.length();i++){
...
QString ServiceQueue::choose(){
    if(this->roomList.empty()){
        return "null";
    }
    int mintime=roomList[0].startTime;
    QString minroom=roomList[0].roomId;
    for(int i=0;i<roomList.length();i++){
        if(roomList[i].startTime<mintime){
            mintime=roomList[i].startTime;
            minroom=roomList[i].roomId;
        }
    }
    return minroom;
}
