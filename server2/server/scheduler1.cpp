...
bool Scheduler::requestTempUp(QString roomId)
{
     for(int i=0; i<airConditioners.size(); i++){
         ...
         if(ac->roomId == roomId && ac->mode == 0){  /* 制冷模式 */
             if(ac->currentTemp <= (defaultTargetTemp-TEMP_CHANGE)){  /*定时器为6s,每次回温0.05度*/

                 ac->currentTemp = defaultTargetTemp;
             }
             else{
             ...
             if(ac->currentTemp >= (defaultTargetTemp-TEMP_CHANGE)){
                 ac->currentTemp = defaultTargetTemp;
             }
             else{
                 ac->currentTemp -= TEMP_CHANGE;
...
void Scheduler::rewarm()
{
    ...
    for(int i=0; i<airConditioners.size(); i++){
        ...
        if(ac->isStart == false){
            /* 当前温度不等于默认温度，需要回温 */
            if(ac->currentTemp != defaultTargetTemp){
                requestTempUp(ac->roomId);
            }
            /* 当前温度与目标温度超过1度，需要重新启动空调 */
            if(abs(ac->currentTemp - ac->targetTemp) >= 1){
    ...
    for(it=serviceQueue->roomList.begin();it!=serviceQueue->roomList.end();it++){
        QString id=it->roomId;
        QList<AirCondition*>::iterator it1;
        AirCondition* ac;
        for(it1=airConditioners.begin();it1!=airConditioners.end();it1++){
            ac=*it1;
            if(ac->roomId==id){
                break;
            }
...
QJsonObject Scheduler::acAnalyse(QJsonObject msg)
{
    ...
    if(request[2] == "init"){
        ...
        double initTemp = data["initTemp"].toDouble();
        /* 新建一个空调对象 */
